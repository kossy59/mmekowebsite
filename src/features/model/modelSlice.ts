...
export const createmodel = createAsyncThunk("model/createmodel", async (data: any) => {
  try {
    const formData = new FormData();
    data.photolink?.forEach((img: File) => formData.append("modelfiles", img));
    formData.append("data", JSON.stringify({ /* relevant fields */ }));
    formData.append("token", data.token);
    data.photolink?.forEach((img: File) => formData.append("modelFiles", img));
    const res = await axios.put(`${URL}/model`, formData, { headers: { "Content-Type": "multipart/form-data" } });
...
export const getmymodelbyid = createAsyncThunk("model/getmymodelbyid", async (data: any) => {
  try {
...
export const updatemodel = createAsyncThunk("model/updatemodel", async (data: any) => {
  try {
    const formData = new FormData();
    const updateData = { /* map your update payload */ };
    formData.append("data", JSON.stringify(updateData));
    formData.append("token", data.token);
    data.photolink?.forEach((file: File) => formData.append("updateModelPhotos", file));
    const res = await axios.post(`${URL}/editmodel`, formData, { headers: { "Content-Type": "multipart/form-data" } });
    if (res.status !== 200) throw "Error updating";
    return res.data;
  } catch (err) { catchError(err); }
});

export const deletemodel = createAsyncThunk("model/deletemodel", async (data: any) => {
  try {
...
export const getverifyhost = createAsyncThunk("model/getverifyhost", async (data: any) => {
  try {
...
const model = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(createmodel.pending, (state) => (state.modelpoststatus = "loading"))
    ...
    builder
      .addCase(getverifyhost.pending, (state) => (state.Listofhoststatus = "loading"))
