...
export default function ModelPage() {
  ...
  const ListofVerihost = useSelector((state: any) => state.model.ListofLivehost);
  const Listofhoststatus = useSelector((state: any) => state.model.Listofhoststatus);

  const [searchQuery, setSearchQuery] = useState("");
  const [nameSearchQuery, setNameSearchQuery] = useState("");
  const [hosttypeSearchQuery, setHosttypeSearchQuery] = useState("");
  const [locationSearchQuery, setLocationSearchQuery] = useState("");
  const [genderSearchQuery, setGenderSearchQuery] = useState<string | null>(null);
  ...
  const filterModels = ListofVerihost.filter((model: any) => {
    const matchesName = nameSearchQuery ? model.name.toLowerCase().includes(nameSearchQuery.toLowerCase()) : true;
    const matchesHostType = hosttypeSearchQuery ? model.hosttype.toLowerCase().includes(hosttypeSearchQuery.toLowerCase()) : true;
    const matchesGender = genderSearchQuery ? model.gender.includes(genderSearchQuery) : true;
    const matchesLocation = locationSearchQuery ? model.location.toLowerCase().includes(locationSearchQuery.toLowerCase()) : true;
  ...
  useEffect(() => {
    if (mymodelstatus !== "loading") {
      dispatch(getmymodel({ userid: user?.userID, token }));
    }
  ...
  const renderModels = () => {
          ...
          <div className="w-full p-4 space-y-4">
            <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
              {Array(10).fill(0).map((_, index) => (
                <div key={index} className="relative flex flex-col items-center p-4 bg-[#121212] rounded-lg">
    ...
    return (
      <ul className="grid grid-cols-2 gap-2 mb-12 md:grid-cols-5">
        {[...onlineModels, ...offlineModels].map((value: any) => (
          value.verify === "live" && (
  ...
  return (
    <div className="px-4 mt-16 bg-black sm:mx-10">
          ...
          <>
            {categoryButton !== " " && (
              <div className="flex items-center">
                ...
                />
                <h3 className="font-bold text-green-600 text-md">#{categoryButton.toLowerCase()}</h3>
              </div>
            )}
              ...
              <>
                <div className="flex items-center justify-between px-2 ml-1 mr-1 text-xs font-bold text-white">
                  {[
                    ...
                    >
                      <img src={item.icon} alt={item.label} className="object-cover w-5 h-5 md:w-7 md:h-7" />
                ...
                <SearchInput
                  ...
                  setNameSearchQuery={(e: any) => setNameSearchQuery(e.target.value)}
      ...
      <Modal isOpen={isModalOpen} title="All Countries" onClose={toggleFullPageModal}>
        <input
