...
export const Modelnotify = ({children}: {children: React.ReactNode}) => {
  ...
  useEffect(()=>{divFocusRef.current && divFocusRef.current.focus()},[])

  return (
    <div className="w-full px-4 pt-8 h-screen flex flex-col contain-content">
...
function HeadBtn({label, route}: {label: string, route: string}){
  ...
  return <Link href={route} className={`w-full font-bold transition-all duration-500 p-2 border border-gray-800 text-center text- text-slate-400 rounded-2xl hover:bg-gray-900 ${endpoint ? "bg-gray-800" : "bg-transparent"}`}>
           {label}
          </Link>
}
